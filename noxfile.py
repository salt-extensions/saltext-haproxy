...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/haproxy/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/haproxy/*")
            ...
            #    "--include=src/saltext/haproxy/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/haproxy/config/schemas",
    )
